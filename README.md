...
| Статус | Описание |
|--------|----------|
| `pending` | В ожидании (ставится автоматически при создании) |
| `in_progress` | В процессе |
| `done` | Выполнено |
| `not_taken` | Не взята в работу (сотрудник отказался, нужен комментарий) |
...
| status | pending / in_progress / done / not_taken | Фильтр по статусу |
...
Сотрудник может поставить один из трёх статусов: `in_progress`, `done`, `not_taken`.
...
**Отказаться от задачи** — комментарий с причиной обязателен:

```
curl -X PATCH http://localhost/api/tasks/1/status/ \
  -H "Authorization: Bearer <ACCESS_TOKEN>" \
  -H "Content-Type: application/json" \
  -d '{"status": "not_taken", "comment": "Загружен другой задачей"}'
```
 
Статус `pending` («в ожидании») — технический. Его нельзя поставить вручную — ни сотрудник, ни руководитель. Он присваивается автоматически при создании задачи.
...
```
{
  ...
  "suggested_assignees": [
    {"id": 3, "full_name": "Петров Пётр", "active_tasks": 0}
  ]
}
```
