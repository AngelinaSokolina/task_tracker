...
class AuthTests(APITestCase):
    ...
    def test_token_obtain_success(self):
        """Успешный логин по телефону и паролю."""
    ...
    def test_me_requires_auth(self):
        """GET /api/users/me/ без авторизации — 401."""
        url = reverse('users-me')
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)
...
class RegisterEmployeeTests(APITestCase):
    ...
    def test_manager_can_register_employee(self):
        """Руководитель может зарегистрировать сотрудника."""
        self.client.force_authenticate(self.manager)
        ...
        # Проверяем, что можно залогиниться с этим паролем
    ...
    def test_employee_cannot_register(self):
        """Сотрудник не может регистрировать других — 403."""
    ...
    def test_anonymous_cannot_register(self):
        """Аноним не может регистрировать — 401."""
        url = reverse('users-register-employee')
        response = self.client.post(url, {
            'phone': '+79994445566',
            'full_name': 'Кто-то',
