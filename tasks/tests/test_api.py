...
class TaskAPITests(APITestCase):
    def setUp(self):
        self.manager = CustomUser.objects.create_user(
            phone='+79990000000', password='pass',
            full_name='Руководитель', role=CustomUser.Role.MANAGER,
        )
        self.other_manager = CustomUser.objects.create_user(
            phone='+79990000099', password='pass',
            full_name='Другой Руководитель', role=CustomUser.Role.MANAGER,
        )
        self.employee = CustomUser.objects.create_user(
            phone='+79991111111', password='pass',
            full_name='Сотрудник', role=CustomUser.Role.EMPLOYEE,
        )
        self.other_employee = CustomUser.objects.create_user(
            phone='+79992222222', password='pass',
            full_name='Другой Сотрудник', role=CustomUser.Role.EMPLOYEE,
        )

        # Задачи для тестов
        self.task_for_employee = Task.objects.create(
            title='Задача для сотрудника', type=TaskType.URGENT,
            assignee=self.employee, author=self.manager,
            date=date(2026, 9, 15),
        )
        self.task_for_other = Task.objects.create(
            title='Задача для другого', type=TaskType.DAILY,
            assignee=self.other_employee, author=self.manager,
            date=date(2026, 9, 15),
        )

    def test_anonymous_cannot_list_tasks(self):
        """Аноним не видит задачи — 401."""
    ...
    def test_manager_can_create_task(self):
        """Руководитель может создать задачу."""
        self.client.force_authenticate(self.manager)
    ...
    def test_employee_can_set_status(self):
        """Сотрудник может изменить статус своей задачи."""
        self.client.force_authenticate(self.employee)
    ...
    def test_manager_can_delete_task(self):
        """Руководитель может удалить задачу."""
        self.client.force_authenticate(self.manager)
...
class CommentAPITests(APITestCase):
    def setUp(self):
        ...
        self.task = Task.objects.create(
            title='Задача', type=TaskType.URGENT,
            assignee=self.employee, author=self.manager,
            date=date(2026, 9, 15),
        )

    def test_add_comment(self):
        """Сотрудник может добавить комментарий к задаче."""
        self.client.force_authenticate(self.employee)
