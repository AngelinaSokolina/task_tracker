...
from tasks.models import Task, Comment, TaskType, TaskStatus
...
class TaskModelTests(TestCase):
    ...
    def test_create_comment(self):
        task = Task.objects.create(
            title='Задача', type=TaskType.INFO,
    ...
    def test_task_ordering(self):
        ...
        t1 = Task.objects.create(
            title='Старая', type=TaskType.DAILY,
            assignee=self.employee, author=self.manager,
            date=date(2026, 9, 1),
        )
        t2 = Task.objects.create(
            title='Новая', type=TaskType.DAILY,
            assignee=self.employee, author=self.manager,
            date=date(2026, 9, 20),
        )
